// Package cli assembles the opennotify command tree from the opennotify
...
// NewApp assembles the kit application from the opennotify domain. The
...
//
// To add a command, declare it in opennotify/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := opennotify.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(opennotify.Domain{}).Register(app)
