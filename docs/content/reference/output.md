...
describes what users get. Pick a format with `-o`, or let opennotify choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
opennotify <command> -o table   # aligned columns for reading
opennotify <command> -o jsonl   # one JSON object per line, for piping
opennotify <command> -o json    # a single JSON array
opennotify <command> -o csv     # spreadsheet friendly
opennotify <command> -o tsv     # tab-separated
opennotify <command> -o url     # just the URL column
opennotify <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
opennotify <command> --fields id,title,url
```
...
```bash
opennotify <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
opennotify <command>            # a table, because this is a terminal
opennotify <command> | wc -l    # JSONL, because this is a pipe
```
